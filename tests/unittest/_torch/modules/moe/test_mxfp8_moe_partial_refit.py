...
requires_mxfp8_moe = pytest.mark.skipif(
    ...
    or not hasattr(torch.ops.trtllm, "block_scale_interleave"),
...
def _rlhf_finalize(module) -> None:
    ...
    for mod in module.modules():
        ...
        if hasattr(mod, "post_load_weights") and not getattr(
                mod, "_weights_removed", False):
...
def _refit(module, buckets: List[Dict[str, torch.Tensor]]) -> None:
    ...
    for mod in module.modules():
        if hasattr(mod, "pre_reload_weights") and not getattr(
                mod, "_weights_removed", False):
...
def _bits(t: torch.Tensor) -> torch.Tensor:
    ...
    """
    return t.view(torch.uint8) if t.dtype.itemsize == 1 else t.contiguous().view(
        torch.uint8)
...
def _mxfp8_expert_weights(generator: torch.Generator):
    ...
    for e in range(NUM_EXPERTS):
        for leaf, shape in (("w1", (INTERMEDIATE_SIZE, HIDDEN_SIZE)),
                            ("w3", (INTERMEDIATE_SIZE, HIDDEN_SIZE)),
                            ("w2", (HIDDEN_SIZE, INTERMEDIATE_SIZE))):
            w = torch.randint(0, 240, shape, generator=generator, device="cuda",
            ...
            sf = torch.randint(110, 140, (shape[0], shape[1] // BLOCK_SIZE),
...
@requires_mxfp8_moe
def test_mxfp8_moe_quant_method_is_selected():
    """Guard the premise: this module must actually use the MXFP8 method."""
    module = _make_module()
    assert isinstance(_backend(module).quant_method, MXFP8CutlassFusedMoEMethod)
...
def test_mxfp8_bucketed_refit_matches_single_shot(bucket_order):
    ...
    expected = _fresh_load(_make_module(), weights)
    ...
    buckets = {
        ...
        "per_leaf": [_split(weights, {"w1"}), _split(weights, {"w3"}),
                     _split(weights, {"w2"})],
    }[bucket_order]
...
@requires_mxfp8_moe
def test_mxfp8_repeated_finalize_is_noop():
...
@requires_mxfp8_moe
def test_mxfp8_two_sequential_refits_match_single_shot():
    ...
    expected_b = _fresh_load(_make_module(), weights_b)
